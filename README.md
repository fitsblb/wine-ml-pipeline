...
pip install -r requirements.txt
pip install -e .
python main.py   # builds data splits, trains model, writes metrics/diagnostics
python app.py    # serve API on :8000



Predict (local)
# Bash/WSL
python sample_payload.py | curl -s -X POST http://localhost:8000/predict -H "Content-Type: application/json" -d @-
# PowerShell
python .\sample_payload.py | curl.exe -s -X POST http://localhost:8000/predict -H "Content-Type: application/json" -d @-
...
artifacts/             # outputs (data splits, model, metrics, plots)
tests/                 # CI tests

Configuration

Edit config/config.yaml; paths are relative to config/ and resolved to absolute by loader.

Edit params.yaml to switch model or scaler.

API

GET /health → {status: "ok"}

POST /predict → { "data": {feat: val, ...} } or a list under data.

Keys must match artifacts/model_trainer/features.json.
...
Wrong columns after read → set io.csv_sep: ";" in config.yaml.

ModuleNotFoundError: datascience → pip install -e . and check src/datascience/__init__.py.

In Docker, missing components → ensure src/datascience/components/__init__.py exists.
...
See [src/datascience/README.md](src/datascience/README.md) for details on pipeline modules, config loaders, and usage examples.

---

## 🏷️ Versioning

We follow [Semantic Versioning (SemVer)](https://semver.org/) for releases. Tags are in the format `vMAJOR.MINOR.PATCH` (e.g., `v1.2.3`).
